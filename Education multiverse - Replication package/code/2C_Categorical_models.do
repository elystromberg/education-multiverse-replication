...
keep edulvlter eiscedter edulvlterpd eiscedterpd edulvlterf eiscedterf edulvlterm eiscedterm ///  
age2465 age2535 age2565 age2569 age2570 age2574 age25pl age2746 ///
age2747 age2855 age2933 age30pl brncntr cntry gndr
 
compress
save 						"ESS_1-10_odds.dta", replace
use 						"ESS_1-10_odds.dta"
...
scalar aa = 0
	foreach c_educ in edulvlter eiscedter { 
		scalar aa = aa + 1

* Setting dominance, the choice here should define the varlist in the next loop
scalar bb = 0
	foreach par_meas in "edulvlterpd eiscedterpd" "edulvlterf eiscedterf" "edulvlterm eiscedterm" {  
		scalar bb = bb + 1
...
capture logit `c_educ' `p_educ'  if !missing(`c_age') `brnfrg' & cntry == "`country'" & gndr== `gender'
local ++rrr
display `rrr'
if _rc!=0 {
		scalar hh = .
...
svmat m
drop in 1
save						"models_odds.dta", replace
use							"models_odds.dta", clear

*------------------------------------------------------*
* Label data 
*------------------------------------------------------*

label data "This file contains multiverse results for logistic regression models"
...
label define var3 1 "ISCED" 2 "ES-ISCED"
label values m3 var3

label define var4 1 "Dominant" 3 "Father" 4 "Mother"
label values m4 var4

label define var5 1 "Parental ISCED" 2 "Parental ES-ISCED"
...
save						"models_odds_raw.dta", replace
use							"models_odds_raw.dta", clear
